...
from _shared import (
    ...
    _DEMO_KEY, _NO_FILE, PRESET_COLORS, PLOT_TYPES, TRANSFORMS,
    apply_transform, _demo_data,
)

st.set_page_config(page_title="FCSViz – Plot", layout="wide")
init_session_state()
...
# ---------------------------------------------------------------------------
# Sidebar — upload + gate tree + export
# ---------------------------------------------------------------------------
with st.sidebar:
...
def _render_drawing_chart(r, c, tool):
    ...
    if tool == "rectangle":
        fig.update_layout(dragmode="select", modebar={"remove": _DRAW_MODEBAR_REMOVE})
        event = st.plotly_chart(
        ...
        verts = list(st.session_state.gate_vertices)
        draw_key = f"chart_{r}_{c}_draw_{len(verts)}"
        fig.update_layout(clickmode="event+select", modebar={"remove": _DRAW_MODEBAR_REMOVE})
        event = st.plotly_chart(
            fig, use_container_width=True, key=draw_key,
            on_select="rerun", selection_mode="points",
        )
...
# ---------------------------------------------------------------------------
# Export sidebar (defined after export helpers)
# ---------------------------------------------------------------------------
with st.sidebar:
    st.divider()
    st.write("**Export**")
